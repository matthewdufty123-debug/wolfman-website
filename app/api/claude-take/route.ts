...
async function requirePostOwner(postId: string) {
  ...
  if (!session?.user?.id) return { error: 'Unauthorised', status: 401, userId: null }
  const [post] = await db.select({ authorId: posts.authorId }).from(posts).where(eq(posts.id, postId))
  if (!post) return { error: 'Not found', status: 404, userId: null }
  if (post.authorId !== session.user.id) return { error: 'Forbidden', status: 403, userId: null }
...
export async function POST(request: Request) {
  ...
  const dataCompleteness = morning && hasEvening ? 'post_morning_evening' : morning ? 'post_morning' : 'post_only'
  ...
  try {
    message = await getAnthropic().messages.create({
      model: MODEL,
      max_tokens: 600,
      system: `You are Claude, reflecting on a Wolfman user's day. They write a morning intention each day — a story, a reflection, a lesson — and return in the evening to log how it all actually went.

You will receive everything captured about this day: the morning intention post, their morning state (routine, brain and body scales), and their evening reflection.
