...
// ── Career ───────────────────────────────────────────────────────────────────
// Matthew's career timeline — roles, achievements, and manual skills.
// Achievements derive skill tags via the skillTags text[] array.
// Manual skills cover anything not tied to a specific achievement.
// WOLF|BOT commentary stored per-record for public display.

export const careerRoles = pgTable('career_roles', {
  id:             uuid('id').primaryKey().defaultRandom(),
  title:          text('title').notNull(),
  company:        text('company').notNull(),
  employmentType: text('employment_type').notNull(),            // 'employed' | 'self-employed'
...
export const careerAchievements = pgTable('career_achievements', {
  ...
  theme:          text('theme').notNull(),                      // 'change-management' | 'data-analytics' | 'operational'
  description:    text('description').notNull(),
  skillTags:      text('skill_tags').array(),                   // text[] — skill names derived from this achievement
  sortOrder:      smallint('sort_order').notNull().default(0),
  wolfbotComment: text('wolfbot_comment'),
  createdAt:      timestamp('created_at').notNull().defaultNow(),
}, (table) => [
  index('career_achievements_role_idx').on(table.roleId),
  index('career_achievements_theme_idx').on(table.theme),
])
...
export const careerSkills = pgTable('career_skills', {
  ...
  theme:          text('theme').notNull(),                      // 'change-management' | 'data-analytics' | 'operational'
  source:         text('source').notNull().default('manual'),   // always 'manual' — derived skills come from achievement skillTags
  firstUsedDate:  text('first_used_date'),                      // "YYYY-MM" — when the skill was first used
  description:    text('description'),
  wolfbotComment: text('wolfbot_comment'),
  createdAt:      timestamp('created_at').notNull().defaultNow(),
})
